...
namespace PixelCrushers.DialogueSystem
{
    ...
    [CustomFieldTypeService.Name("HexColor")]

    // STEP 2: Rename the class by changing TemplateType to your type name:
    public class CustomFieldType_HexColor : CustomFieldType
    {

        private static Color GenerateColor(string currentValue)
        {
            var color = Color.white;

            if (string.IsNullOrWhiteSpace(currentValue) == false)
                ColorUtility.TryParseHtmlString(currentValue, out color);
        ...
        {
            Color color = GenerateColor(currentValue);
            var newColor = EditorGUILayout.ColorField(color);
            
            //var newString = ColorUtility.ToHtmlStringRGB(newColor);
            return EditorGUILayout.TextField(Tools.ToWebColor(newColor));
